...
cap = cv.VideoCapture('videos\jump_01.mp4')
...
print('Motion Energy Images...')
print(len(MEI))

plt.figure(figsize=(12, 8))
for i in range(15):
    plt.subplot(4, 5, i + 1)
    plt.imshow(MEI[i], cmap='gray')
...
for i in range(15):
    plt.subplot(4, 5, i + 1)
    plt.imshow(MEI_cleaned[i], cmap='gray')
    plt.title(f'Frame {i + 1}')
    plt.axis('off')
...
for i in range(15):
    ...
    action_01_sds.append(hu_moments)
    print(hu_moments)
...
# # ============================================== #
# #   step 5: copmare shape descriptors (hu-moments)
# #           between two actions using difference
# #           measure... MSE 

# # method from MSE from internet (not re-inventing the wheel)
def mse(actual, pred): 
    actual, pred = np.array(actual), np.array(pred)
    return np.square(np.subtract(actual,pred)).mean() 

# # now.. we can compare two actions :)